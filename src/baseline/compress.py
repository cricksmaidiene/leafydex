...
image_compress_map_tuples: list[tuple[str, str]] = [(k,v) for k,v in image_compress_map.items()]
...
with ThreadPoolExecutor(max_workers=10) as executor:
    executor.map(
        lambda path_tuple: compress_image(*path_tuple), 
        image_compress_map_tuples
    )